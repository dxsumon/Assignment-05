...
for (const btn of allBtn) {
    btn.addEventListener('click', function (event) {
        if (count < 4 && !btn.classList.contains('bg-green-500')) {
            ...
            setTextValueById('grand-total-price', updateTotalPrice);
                ...
                if (couponCode === 'NEW15' || couponCode === 'Couple 20') {
                    ...
                    document.getElementById('apply-btn').addEventListener('click', function () {
                        if (couponCode === 'NEW15') {
                            ...
                            // Set 15% discount totalPrice 
                            const userGotDiscount = document.getElementById('discount');
                            userGotDiscount.innerText = `Congratulation you got 15% discount`;
                            const p = document.createElement('p');
                            p.innerText = `- ${discountPrice}`;
                            userGotDiscount.appendChild(p);
                        }
                            ...
                            // Set 20% discount totalPrice 
                            const userGotDiscount = document.getElementById('discount');
                            userGotDiscount.innerText = `Congratulation you got 20% discount`;
                            const p = document.createElement('p');
                            p.innerText = `- ${discountPrice2}`;
                            userGotDiscount.appendChild(p);
                        }
